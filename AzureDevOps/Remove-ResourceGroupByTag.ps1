...
workflow Remove-ResourceGroupByTag
{
    ...
    if ( $JobGuid.Length -lt 5 ) {
        ...
        foreach ($Automation in $AutomationResource) {
            if ( $KeepProcessing ) {
                ...
                $Job = Get-AzureRmAutomationJob -ResourceGroupName $Automation.ResourceGroupName -AutomationAccountName $Automation.Name -Id $JobGuid -ErrorAction SilentlyContinue
    ...
    try {
        ...
        $name_rg = @(Get-AzureRmResourceGroup | `
        ...
        if ($tags.Count -gt 0 ) {
            ...
            $tag_rg = @(Get-AzureRmResourceGroup  -Tag $Tags | select ResourceGroupName)
            $tag_rg
        ...
        else
        {
            ...
            # Remove the resource groups
                ...
                foreach -parallel ($resourceGroup in $groupsToRemove) {
                    ...
                    if ($null -eq (Get-AzureRmResourceGroup -Name $($resourceGroup.ResourceGroupName) -ErrorAction SilentlyContinue) ) {
