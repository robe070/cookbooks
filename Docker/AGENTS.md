...
- Generate a new `word\document.xml` from the markdown content, preserving the `Title`, `Heading1`, and `Heading2` style ids, turning fenced code blocks / inline backticks into monospace runs, converting inline `**bold**` markup into bold runs, and packaging local markdown image references into `word\media` with matching relationship and content-type entries.
